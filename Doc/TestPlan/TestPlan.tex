...
the system will be tested.In \wss{proof read} this plan, most of the testing
...
The system will be run on a big data platform (Spark), in order to \wss{replace
  ``in order to'' with ``to''} obtain the more accurate results. In this case, we need to setup a distributed system to support Spark.
...
\wss{good idea to use PyChecker}
...
of the price and number of digits of decimals. Shown in Figure 1 \wss{End
  sentences with a period.} \wss{Use cross-referencing in LaTeX, rather than
  hard coding the values.}  \wss{You shouldn't be saying ``such as'' in your
  test description.  You should describe everything that you are looking for.
  What do you do if the data is in the incorrect format?  What could go wrong?
  Where are the test cases to ensure that your software behaves correctly in
  those situations?}

\wss{Your test description is not specific enough.  What will the contents of
  the file b?  If the data is difficult to specify, maybe you could point to a
  sample file in your repo that you are going to use?  Your test plan document
  could then point to the file in the repo.}
...
\wss{This description doesn't have enough detail.  What is the array that will
  be sent to each worker?  What is the mark?  Provide enough information that
  someone (other than you) could write the test.}
...
Type: Functional, Manual, Static etc. \wss{You shouldn't use etc.\ here}
Initial State: NA
...
\wss{If SVM is a unit test, then it shouldn't also appear here.}
...
\wss{Not enough detail.  How many workers are you going to experiment with?
  What test case are you going to get them to perform?  What are the axes on the
  graph you are going to produce as output of the testing?}
...
\wss{If you use shorter names for the column headings, it will fit better on the
  page.}
...
\wss{As mentioned previously, you need to be more specific with your test
  cases.  What are the values of the inputs and the expected outputs?  You want
  test cases that someone else can directly build from the information you have
  provided.}
